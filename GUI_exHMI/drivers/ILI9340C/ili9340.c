...
uint8_t ili9340_start(emHMIGUI* gui)
{
...
uint8_t ili9340_sleep(emHMIGUI* gui)
{
    return M_OK;
}
uint8_t ili9340_awake(emHMIGUI* gui)
{
    return M_OK;
}
uint8_t ili9340_set_backlight(emHMIGUI* gui, uint8_t val)
{
#if ILI9340_LED_USE_PWM
...
uint8_t ili9340_spi_txhalfcplt(emHMIDriver* d)
{
    if(d->posy < d->lcd_height)
	emHMI_render(d->gui, 1);
    return M_OK;
}
uint8_t ili9340_spi_txcplt(emHMIDriver* d)
{
//    _ili9340_cs(1);

    uint8_t dr = 0;
    emHMIBuffer *bu = d->gui->buffer;
